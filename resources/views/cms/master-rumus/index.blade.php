...
@section('content')
    <div class="card border-0 shadow-sm">
        <div class="card-header bg-white py-3 border-bottom">
            <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
                ...
                <div class="d-flex gap-2">
                    <form method="GET" action="{{ route('cms.master-rumus.index') }}" class="d-flex gap-2">
                        <input type="text" name="search" class="form-control form-control-sm" placeholder="Cari rumus..." value="{{ $search ?? '' }}">
                        <button type="submit" class="btn btn-sm btn-outline-secondary"><i class="bi bi-search"></i></button>
                        @if(!empty($search))
                            <a href="{{ route('cms.master-rumus.index') }}" class="btn btn-sm btn-outline-danger"><i class="bi bi-x-lg"></i></a>
                        @endif
                    </form>
        ...
        @if($rumus->hasPages())
        <div class="card-footer bg-white border-top py-3">{{ $rumus->appends(['search' => $search])->links('pagination::bootstrap-4') }}</div>
        @endif
    </div>
@endsection
