...
<head>
    <meta charset="utf-8">
    <title>Dashboard - {{ $client->nama_client }}</title>
    <style>
...
<body>
    ...
    <table class="client-info">
        <tr><td style="width:120px"><strong>Nama</strong></td><td>{{ $client->nama_client }}</td></tr>
        <tr><td><strong>NPWP</strong></td><td>{{ $client->npwp ?? '-' }}</td></tr>
        <tr><td><strong>Tipe</strong></td><td>{{ $tipeName }}</td></tr>
        <tr><td><strong>Tahun</strong></td><td>{{ $tahun }}</td></tr>
    </table>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                @if($tab['key'] === 'induk')
                <td class="text-end">{{ $row['omset'] ? ($row['totalBrutoAkum'] ?: '-') : '-' }}</td>
                @endif
    ...
    <table>
        ...
        @foreach($allTabs as $tab)
        <tr><td class="text-muted" style="padding-left:20px">{{ $tab['npwp'] ?: $tab['label'] }}</td><td class="text-end text-muted">Rp {{ number_format($tab['total_omset'], 0, ',', '.') }}</td></tr>
        @endforeach
    </table>
</body>
</html>
